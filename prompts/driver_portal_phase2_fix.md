...
Your ONLY job is to create/edit the files listed under "Allowed files". **Do NOT run `scripts/ai/full_system_dry_run.sh`, `npm run test:rules`, `firebase emulators:*`, or start any emulator/dev server.** The harness runs the test gate AFTER you finish, in an environment that can bind ports (your sandbox cannot — the Firestore emulator will fail with `EPERM`/`port taken` and is NOT a signal to stop). Make the edits, then end. Use `node --check` for JS syntax only if you wish; nothing else.
